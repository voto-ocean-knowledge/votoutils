...
import json
import logging
from votoutils.alerts.alarm_utils import (
    setup_logger,
    format_alarm,
    secrets_dict,
    contact_pilot,
...
class Dispatcher:
    def __init__(self, platform_id):
        ...
        setup_logger(
            platform_id, self.alarm_log, formatter=format_alarm, level=logging.INFO
        )

    def load_alarm_log(self):
        if Path(self.alarm_log).exists():
            self.df_alarm = pd.read_csv(
                self.alarm_log,
                names=[
                    "datetime",
                    "glider",
    ...
    def check_comm_log(self):
        ...
        if df.iloc[-1]["datetime"] < datetime.datetime.now() - datetime.timedelta(
            hours=6
        ):
        ...
        if not ddict["alarm"]:
            _log.info(
                f"Alarm cleared {self.platform_id} M{ddict['mission']} cycle {ddict['cycle']}"
            )
            return False
    ...
    def mail_alarm(self):
        ...
        email_dict = {
            "cycle": alarm_tuple[1],
            "glider": self.platform_num,
            "mission": alarm_tuple[0],
            "security_level": alarm_tuple[2],
            "alarm": True,
        }
        ...
        if self.alarm_dict["mission"] > email_dict["mission"]:
            _log.info(
                f"stale email. Skipping. mission: {self.alarm_dict['mission']} vs {email_dict['mission']}, "
                f"cycle  {self.alarm_dict['cycle']} vs {email_dict['cycle']}",
            )
            return False

        if (
            self.alarm_dict["mission"] >= email_dict["mission"]
            and self.alarm_dict["cycle"] >= email_dict["cycle"]
        ):
            _log.info(
                f"stale email. Skipping. mission: {self.alarm_dict['mission']} vs {email_dict['mission']}, "
                f"cycle  {self.alarm_dict['cycle']} vs {email_dict['cycle']}",
...
if __name__ == "__main__":
    ...
    fail_count += 1
    if fail_count == 10:
        mailer("failed-alerts", "automated alerts system has failed. Switch over to backup system e.g. IFTTT", mail_recipient)
    fail = False
    with open(fail_file, 'w') as fout:
        fout.write(str(fail_count))
    try:
